...
export default function TagsPage() {
  ...
  const fetchTags = useCallback(async () => {
    ...
    try {
      ...
      const data: Tag[] = Array.isArray(res.data) ? res.data : (res.data?.data ?? [])
      setAll(data)
  ...
  function applyFilter(data: Tag[], q: string) {
    const q2 = q.trim().toLowerCase()
    const result = q2
      ? data.filter(t => t.name.toLowerCase().includes(q2) || t.slug.toLowerCase().includes(q2))
      : data
    setFiltered(result)
    setPage(1)
  }
  ...
  function openCreate() { setFname(""); setFerr(""); setModal({ mode: "create" }) }
  function openEdit(tag: Tag) { setFname(tag.name); setFerr(""); setModal({ mode: "edit", tag }) }
  function closeModal() { if (!busy) setModal({ mode: "none" }) }

  async function handleCreate() {
    if (!fname.trim()) { setFerr("Name is required."); return }
    setBusy(true); setFerr("")
    try {
      await api.post("/tags", { name: fname.trim() })
      setModal({ mode: "none" })
      showToast(`"${fname.trim()}" created.`, true)
      fetchTags()
    } catch (err: any) {
      setFerr(err?.response?.data?.message || "Failed to create tag.")
  ...
  async function handleUpdate() {
    ...
    setBusy(true); setFerr("")
    try {
      await api.put(`/tags/${modal.tag.id}`, { name: fname.trim() })
      setModal({ mode: "none" })
      showToast(`"${fname.trim()}" updated.`, true)
      fetchTags()
    } catch (err: any) {
      setFerr(err?.response?.data?.message || "Failed to update tag.")
    } finally { setBusy(false) }
  }
  ...
  async function handleDelete() {
    ...
    try {
      ...
      setModal({ mode: "none" })
      showToast(`"${tag.name}" deleted.`, true)
      fetchTags()
    } catch (err: any) {
  ...
  const columns = [
    {
      ...
      render: (row: Tag) => (
        <div className="col-tag">
          <span className="col-tag-hash">#</span>
          <div className="col-tag-info">
            <span className="col-tag-name">{row.name}</span>
            <span className="col-tag-slug">{row.slug}</span>
          </div>
        </div>
      ),
    },
    {
      ...
      render: (row: Tag) => (
        <div className="col-count">
          <span className="col-count-num">{Number(row.postCount ?? 0)}</span>
          <span className="col-count-label">post{Number(row.postCount) !== 1 ? "s" : ""}</span>
        </div>
      ),
    },
    {
      label: "Created", key: "created_at",
      render: (row: Tag) => <span className="col-date">{formatDate(row.created_at)}</span>,
    },
    {
      ...
      render: (row: Tag) => (
        <div className="action-group">
          ...
          <button
            ...
            onClick={() => setModal({ mode: "delete", tag: row })}
            disabled={Number(row.postCount) > 0}
            title={Number(row.postCount) > 0 ? "Remove from posts first" : "Delete"}
          >
    ...
    <div className="tags-page">
      ...
      <div className="page-header">
        <div className="header-left">
          ...
          <p className="page-sub">{all.length} total tag{all.length !== 1 ? "s" : ""}</p>
        </div>
        <div className="header-actions">
          <button className="refresh-btn" onClick={fetchTags} disabled={loading} title="Refresh">
      ...
      <div className="stats-bar">
        {[
          ...
          { label: "With posts", value: all.filter(t => Number(t.postCount) > 0).length,    color: "var(--green)"  },
          { label: "Unused",     value: all.filter(t => Number(t.postCount) === 0).length,  color: "var(--muted)"  },
        ].map(s => (
      ...
      <div className="toolbar">
        <div className="search-wrap">
          ...
          <input className="search-inp" placeholder="Search tags…" value={search} onChange={e => setSearch(e.target.value)}/>
          {search && (
      ...
      <div className="table-card">
        {loading ? (
          <div className="loading-state"><div className="loading-spinner"/><span>Loading tags…</span></div>
        ) : paginated.length === 0 ? (
        ...
        <div className="modal-backdrop" onClick={closeModal}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              ...
              <div>
                <h3 className="modal-title">{modal.mode === "create" ? "New Tag" : "Edit Tag"}</h3>
                <p className="modal-subtitle">{modal.mode === "create" ? "Add a new tag" : `Editing "${(modal as any).tag.name}"`}</p>
              </div>
            ...
            <div className="modal-footer">
              ...
              <button className="modal-btn modal-btn--accent" onClick={modal.mode === "create" ? handleCreate : handleUpdate} disabled={busy}>
                {busy ? <><span className="btn-spin"/>Saving…</> : modal.mode === "create" ? "Create Tag" : "Save Changes"}
              </button>
            </div>
          </div>
        </div>
      ...
      {modal.mode === "delete" && (
        <div className="modal-backdrop" onClick={closeModal}>
          <div className="modal modal--sm" onClick={e => e.stopPropagation()}>
            ...
            <h3 className="modal-title" style={{ textAlign: "center", marginBottom: 8 }}>Delete tag?</h3>
            <p className="modal-del-body"><strong>"#{modal.tag.name}"</strong> will be permanently removed.</p>
            <div className="modal-footer">
              <button className="modal-btn modal-btn--ghost" onClick={closeModal} disabled={busy}>Cancel</button>
      ...
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
        :root{--bg:#0a0a0f;--surface:#111118;--elevated:#16161f;--border:rgba(255,255,255,0.07);--border-hover:rgba(255,255,255,0.13);--text:rgba(255,255,255,0.90);--muted:rgba(255,255,255,0.38);--accent:#6366f1;--accent-dim:rgba(99,102,241,0.15);--accent-glow:rgba(99,102,241,0.3);--green:#34d399;--green-dim:rgba(52,211,153,0.12);--red:#f87171;--red-dim:rgba(248,113,113,0.12);--font:'DM Sans',-apple-system,sans-serif;--radius:12px;--t:0.18s cubic-bezier(0.4,0,0.2,1);}
        .tags-page{display:flex;flex-direction:column;gap:20px;font-family:var(--font);animation:page-in 0.4s cubic-bezier(0.4,0,0.2,1) both;}
        ...
        .col-tag{display:flex;align-items:center;gap:10px;}
        .col-tag-hash{font-size:18px;font-weight:700;color:var(--accent);opacity:0.5;font-family:'Syne',sans-serif;line-height:1;}
        .col-tag-info{display:flex;flex-direction:column;gap:2px;}
        .col-tag-name{font-weight:500;color:var(--text);font-size:13.5px;}
        .col-tag-slug{font-size:11px;color:var(--muted);font-family:monospace;}
        ...
        .modal{background:var(--elevated);border:1px solid var(--border-hover);border-radius:18px;max-width:440px;width:100%;animation:slide-up 0.2s cubic-bezier(0.4,0,0.2,1) both;box-shadow:0 24px 80px rgba(0,0,0,0.6);overflow:hidden;}
        .modal--sm{max-width:360px;padding:28px 24px 20px;text-align:center;}
        ...
        .modal-header-icon{width:36px;height:36px;border-radius:9px;background:var(--accent-dim);color:var(--accent);display:flex;align-items:center;justify-content:center;flex-shrink:0;}
        .modal-title{font-family:'Syne',sans-serif;font-size:16px;font-weight:700;color:var(--text);letter-spacing:-0.01em;}
        ...
        .modal-icon-circle{width:52px;height:52px;border-radius:50%;display:flex;align-items:center;justify-content:center;margin:0 auto 14px;}
        .modal-icon-circle--red{background:var(--red-dim);color:var(--red);}
        .modal-del-body{font-size:13.5px;color:var(--muted);line-height:1.6;margin-bottom:18px;}
        .modal-del-body strong{color:var(--text);}
        .modal-footer{display:flex;gap:10px;padding:14px 22px 20px;}
        ...
        @media(max-width:640px){.page-header{align-items:flex-start;flex-direction:column;}.create-btn{width:100%;justify-content:center;}.toolbar{flex-direction:column;align-items:stretch;}.search-wrap{max-width:100%;}.action-group{flex-direction:column;}.btn-action{width:100%;justify-content:center;}}
      `}</style>
    </div>
  )
}