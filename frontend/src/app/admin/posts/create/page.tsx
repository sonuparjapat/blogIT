...
export default function CreatePostPage() {
  ...
  const mediaBadge     = gallery.length + (featuredFile ? 1 : 0)
  const taxonomyBadge  = selectedCategories.length + selectedTags.length

  /* ── slug auto ── */
  useEffect(() => { if (!slugLocked && title) setSlug(slugify(title)) }, [title, slugLocked])

  /* ── media handlers ── */
  function handleFeaturedPick(e: React.ChangeEvent<HTMLInputElement>) {
    const f = e.target.files?.[0]; if (!f) return
    setFeaturedFile(f); setFeaturedPreview(URL.createObjectURL(f))
    e.target.value = ""
  }
  function removeFeatured() { setFeaturedFile(null); setFeaturedPreview("") }
  ...
  function removeGalleryItem(idx: number) { setGallery(prev => prev.filter((_, i) => i !== idx)) }

  /* ── submit ── */
  const submit = useCallback(async (publishNow?: boolean) => {
  ...
  }, [title, slug, excerpt, content, seoTitle, seoDesc, status, isPremium, rt, scheduledAt, featuredFile, gallery, selectedCategories, selectedTags, router])

  /* ── ⌘S shortcut ── */
  useEffect(() => {
    const h = (e: KeyboardEvent) => { if ((e.metaKey || e.ctrlKey) && e.key === "s") { e.preventDefault(); submit() } }
    ...
    <div className="cp">
      ...
      <header className="cp-bar">
        <div className="cp-bar-l">
          ...
          <span className="cp-crumb">New post</span>
        </div>

        {/* tab nav */}
        <nav className="cp-nav">
          {(["write","media","seo","settings"] as Tab[]).map(t => (
            <button key={t} className={`cp-nav-btn ${tab===t?"cp-nav-btn--on":""}`} onClick={() => setTab(t)}>
              {tabIcon(t)}
        ...
        <div className="cp-bar-r">
          ...
          <span className={`cp-status-pill ${status==="published"?"cp-status-pill--live":"cp-status-pill--draft"}`}>
            <span className="cp-dot"/>{status==="published"?"Live":"Draft"}
          </span>
          {isPremium && (
          ...
          <button className="cp-draft-btn" onClick={() => { setStatus("draft"); submit() }} disabled={saving}>
            Save draft
          </button>
          <button className="cp-publish-btn" onClick={() => { setStatus("published"); submit(true) }} disabled={saving}>
            {saving
              ...
              : <>Publish <span className="cp-kbd">⌘S</span></>
            }
          </button>
        </div>
      </header>
          ...
          <div className="cp-write">
            ...
            <div className="cp-title-area">
              ...
              <div className="cp-slug-row">
                ...
                />
                {slugLocked && (
                  <button className="cp-slug-auto" onClick={() => { setSlugLocked(false); setSlug(slugify(title)) }}>↺ auto</button>
                )}
              </div>
            </div>
            ...
            <div className="cp-field">
              ...
              <textarea
                ...
                rows={3}
              />
              <span className="cp-charcount" style={{ color: excerpt.length > 300 ? "#f87171" : "" }}>{excerpt.length}/300</span>
            </div>
          </div>
        )}
        ...
        {tab==="media" && (
          <div className="cp-panel">
            <div className="cp-panel-hd">
              ...
              <p className="cp-panel-sub">Upload a featured image and optional gallery images for this post.</p>
            </div>

            {/* featured */}
            <div className="cp-section">
              <div className="cp-section-label">
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>
                Featured image
              </div>
              {featuredPreview ? (
                <div className="cp-featured">
                  <img src={featuredPreview} alt="" className="cp-featured-img"/>
                  <div className="cp-featured-overlay">
                    <label className="cp-overlay-btn">
                      ...
                      Replace
                      <input type="file" accept="image/*" hidden onChange={handleFeaturedPick}/>
                    </label>
                    <button className="cp-overlay-btn cp-overlay-btn--danger" onClick={removeFeatured}>
                  ...
                  <div className="cp-featured-meta">{featuredFile?.name} · {((featuredFile?.size||0)/1024).toFixed(0)} KB</div>
                </div>
              ) : (
                <label className="cp-dropzone cp-dropzone--lg">
                  <div className="cp-dropzone-icon">
                    <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>
                  </div>
                  <div className="cp-dropzone-title">Drop featured image or click to browse</div>
            ...
            <div className="cp-section">
              <div className="cp-section-label">
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="2" width="20" height="20" rx="2"/><line x1="7" y1="2" x2="7" y2="22"/><line x1="2" y1="7" x2="22" y2="7"/><line x1="2" y1="17" x2="22" y2="17"/><line x1="17" y1="2" x2="17" y2="22"/></svg>
                Gallery
                {gallery.length > 0 && <span className="cp-count-badge">{gallery.length}</span>}
              </div>
              <div className="cp-gallery">
                {gallery.map((g,i) => (
                  <div key={i} className="cp-gallery-item">
                    <img src={g.preview} alt="" className="cp-gallery-img"/>
                    <button className="cp-gallery-rm" onClick={() => removeGalleryItem(i)}>
                ...
                <label className="cp-gallery-add">
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
                  <span>{gallery.length===0?"Add images":"More"}</span>
                  <input ref={galleryInputRef} type="file" accept="image/*" multiple hidden onChange={addGalleryFiles}/>
                </label>
              </div>
              {gallery.length > 0 && (
                <p className="cp-section-note">{gallery.length} image{gallery.length!==1?"s":""} · {(gallery.reduce((a,g)=>a+g.file.size,0)/1024/1024).toFixed(2)} MB total</p>
              )}
            </div>
            ...
            <div className="cp-section">
              ...
              <div className="cp-payload">
                {([
                  ...
                  ["featured_image",  featuredFile?featuredFile.name:null,                      "none"],
                  ["gallery_images[]",gallery.length>0?`${gallery.length} file(s)`:null,       "none"],
            ...
            <div className="cp-seo-grid">
              <div className="cp-seo-left">
                <div className="cp-field">
                  <label className="cp-label">
                    SEO Title
                    <span className={`cp-charcount ${seoTitle.length>60?"cp-charcount--over":""}`}>{seoTitle.length}/60</span>
                  </label>
                  <input className="cp-inp" placeholder="Leave blank to use post title" value={seoTitle} onChange={e => setSeoTitle(e.target.value)}/>
                  <div className="cp-prog"><div className="cp-prog-bar" style={{ width:`${Math.min(100,(seoTitle||title).length/60*100)}%`, background:(seoTitle||title).length>60?"#f87171":(seoTitle||title).length>40?"#34d399":"#a5b4fc" }}/></div>
                </div>
                <div className="cp-field" style={{marginTop:20}}>
                  <label className="cp-label">
                    Meta Description
                    <span className={`cp-charcount ${seoDesc.length>160?"cp-charcount--over":""}`}>{seoDesc.length}/160</span>
                  </label>
                  <textarea className="cp-ta" placeholder="Leave blank to use excerpt" value={seoDesc} onChange={e => setSeoDesc(e.target.value)} rows={4}/>
                  <div className="cp-prog"><div className="cp-prog-bar" style={{ width:`${Math.min(100,(seoDesc||excerpt).length/160*100)}%`, background:(seoDesc||excerpt).length>160?"#f87171":(seoDesc||excerpt).length>80?"#34d399":"#a5b4fc" }}/></div>
                </div>
                {/* SEO checks */}
              ...
              <div className="cp-seo-right">
                ...
                <div className="cp-serp">
                  ...
                  <div className="cp-serp-desc">{seoDescFinal||"Your meta description will appear here. Write something that makes people want to click through and read more."}</div>
                </div>
              </div>
            </div>
          </div>
        ...
        {tab==="settings" && (
          <div className="cp-panel">
            ...
            <div className="cp-settings-grid">
              ...
              <div className="cp-card">
                ...
                <div className="cp-toggle-row" onClick={() => setIsPremium(p => !p)}>
                  ...
                  <div className={`cp-toggle ${isPremium?"cp-toggle--on":""}`}><div className="cp-toggle-knob"/></div>
                </div>
                {isPremium && (
              ...
              <div className="cp-card">
                ...
                <p className="cp-card-hint">Leave empty to publish immediately when status is Published.</p>
                <input type="datetime-local" className="cp-inp cp-inp--dt" value={scheduledAt} onChange={e => setScheduledAt(e.target.value)}/>
                {scheduledAt && <button className="cp-text-btn" onClick={() => setScheduledAt("")}>Clear schedule</button>}
              </div>
              ...
              <div className="cp-card cp-card--full">
                <div className="cp-card-title">
                  ...
                  Categories
                  {selectedCategories.length > 0 && <span className="cp-badge" style={{marginLeft:"auto"}}>{selectedCategories.length}</span>}
                </div>
              ...
              <div className="cp-card cp-card--full">
                <div className="cp-card-title">
                  <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M20.59 13.41l-7.17 7.17a2 2 0 0 1-2.83 0L2 12V2h10l8.59 8.59a2 2 0 0 1 0 2.82z"/><line x1="7" y1="7" x2="7.01" y2="7"/></svg>
                  Tags
                  {selectedTags.length > 0 && <span className="cp-badge" style={{marginLeft:"auto"}}>{selectedTags.length}</span>}
                </div>
            ...
            <div className="cp-save-row">
              <button className="cp-save-draft" onClick={() => { setStatus("draft"); submit() }} disabled={saving}>
                Save as draft
              </button>
              <button className="cp-save-publish" onClick={() => { setStatus("published"); submit(true) }} disabled={saving}>
                {saving ? "Saving…" : saveState==="saved" ? "✓ Saved!" : "Publish post"}
              </button>
            </div>
          </div>
        )}
...
const cpStyles = `
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
  ...
  .cp-crumb{font-size:12.5px;font-weight:500;color:var(--t0);white-space:nowrap;}
  ...
  .cp-pk{flex-shrink:0;width:160px;font-family:monospace;font-size:11.5px;color:var(--acc-l);}
